...
#[test]
fn cli_init_does_not_generate_agents_file() {
    let workspace = TestWorkspace::new();

    let initialized = workspace.run(&["init", "--format", "json"]);
    let initialized: Value = serde_json::from_slice(&initialized.stdout).unwrap();

    assert_eq!(initialized["mode"], "local");
    assert!(initialized.get("agents_file").is_none());
    assert!(!workspace.root().join(".vault/AGENTS.md").exists());

    let human_output = workspace.run(&["init"]);
    assert!(!String::from_utf8_lossy(&human_output.stdout).contains("AGENTS"));
